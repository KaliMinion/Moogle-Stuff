local tbl =
{
	[1] = {
		name = "Moogle Updater",
		status = "open",
		filepath = [[Moogle Updater.lua]],
		table = "return MoogleUpdater",
		url = MoogleLib.API.GitURL("TestFolder/Moogle%20Updater"),
		category = "Core Moogle Module",
		stability = "Core Moogle Module",
		info = [[Downloads scripts, keeps them updated. :P! :o]],
	},
	[2] = {
		name = "Main Window",
		status = "open",
		filepath = [[Main Window.lua]],
		table = "return KaliMainWindow",
		url = MoogleLib.API.GitURL("TestFolder/Main%20Window"),
		category = "Core Moogle Module",
		stability = "Core Moogle Module",
		info = [[The Main Window that all Moogle Scripts share to display the settings for each module.]],
	},
	[3] = {
		name = "MoogleLib",
		status = "open",
		filepath = [[MoogleLib.lua]],
		table = "return MoogleLib",
		url = MoogleLib.API.GitURL("TestFolder/MoogleLib"),
		category = "Core Moogle Module",
		stability = "Core Moogle Module",
		info = [[Where all my Moogle Functions are stored.]],
	},
	[4] = {
		name = "Moogle Text to Speech",
		status = "open",
		filepath = [[Moogle Scripts\Moogle TTS.lua]],
		table = "return MoogleTTS",
		url = MoogleLib.API.GitURL("TestFolder/Moogle%20Scripts/Main%20Window"),
		category = "Utility",
		stability = "WiP but Working",
		info = [[Narrates NPC dialog, as well as some other windows in game.]],
	},
	[5] = {
		name = "Moogle FPS",
		status = "open",
		filepath = [[Moogle Scripts\Moogle FPS.lua]],
		table = "return MoogleFPS",
		url = MoogleLib.API.GitURL("TestFolder/Moogle%20Scripts/Moogle%20FPS"),
		category = "Utility",
		stability = "Working",
		info = [[An FPS overlay like Fraps or Nvidia's GeForce Experience. For people who have issue using Minion with other overlays.]],
	},
	[6] = {
		name = "Moogle PushButton",
		status = "open",
		filepath = [[Moogle Scripts\Moogle PushButton.lua]],
		table = "return MooglePushButton",
		url = MoogleLib.API.GitURL("TestFolder/Moogle%20Scripts/Moogle%20PushButton"),
		category = "Dev",
		stability = "Working",
		info = [[A developer module like the one in Dev, but able to iterate PushButton presses.]],
	},
	[7] = {
		name = "Moogle Tweaks",
		status = "open",
		filepath = [[Moogle Scripts\Moogle Tweaks.lua]],
		table = "return MoogleTweaks",
		url = MoogleLib.API.GitURL("TestFolder/Moogle%20Scripts/Moogle%20Tweaks"),
		category = "Utility",
		stability = "WiP but Working",
		info = [[Navigate to NPCs and trade in items for stuffs, temp name, will make it better later.]],
	},
	[8] = {
		name = "Moogle MultiSession Manager",
		status = "open",
		filepath = [[Moogle Scripts\Moogle MultiSession Manager.lua]],
		table = "return MoogleMultiSessionManager",
		url = MoogleLib.API.GitURL("TestFolder/Moogle%20Scripts/Moogle%20MultiSession%20Manager"),
		category = "Utility",
		stability = "Beta",
		info = [[A module to monitor what other bot instances on your computer are doing, send commands to specific instances or all at the same time.]],
	},
}
return tbl
